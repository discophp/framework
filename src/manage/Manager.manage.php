...
";
    ...
    */
    public static function buildRecord($table){
        ...
        while($row = $result->fetch()){
            ...
            $camelCaseFieldName = toCamelCase($row['Field']);
            $functionName = ucwords($camelCaseFieldName);

            $dataType = 'string';

            if(array_search($rule['type'], ['decimal', 'float', 'double']) !== false){
                $dataType = 'float';
            } else if(stripos($rule['type'], 'int') !== false){
                $dataType = 'int';
            } else if(array_search($rule['type'], ['datetime', 'date', 'time', 'year', 'timestamp']) !== false){
                $dataType = 'string|\DateTime';
            }

            $dataType = $dataType . ($rule['null'] ? '|null' : '');
    ...
    */
    public function get{$functionName}(){
        return \$this->fields['{$row['Field']}'];
    }
    
    /**
    * @param {$dataType} \${$camelCaseFieldName}
    * @return \$this
    */
    public function set{$functionName}(\${$camelCaseFieldName}){
        \$this->fields['{$row['Field']}'] = \${$camelCaseFieldName};
        return \$this;
    }
