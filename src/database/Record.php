...
abstract class Record implements \ArrayAccess {
    ...
    */
    public static function find($where, $select = null){
        ...
        if(!is_array($where)){
            throw new \InvalidArgumentException("{$class}::find() Paramater 1 `\$where` must be an array");
        }//if
    ...
    */
    public static function findAll($where, $select = null){
        ...
        if(!is_array($where)){
            throw new \InvalidArgumentException("{$class}::findAll() Paramater 1 `\$where` must be an array");
        }//if
    ...
    */
    public function validate($field, $v){
        ...
        //raw type?
        if(is_array($v) && count($v) == 1 && isset($v[\Disco\classes\PDO::$rawValueKey])){
            return true;
        }//if
