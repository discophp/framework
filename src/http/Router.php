...
class Router {
    ...
    */
    public static $base = '\Disco\http\Router';
    ...
    */
    ...
    /**
     * @var string|array $middleware The middleware that should be applied to the route.
     */
    private $middleware = [];


    /**
     * @var array $executedMiddleware The middleware that was executed on the route. Used after the router has been executed
     * to check for a terminate method on each of the middleware's.
     */
    private $executedMiddleware = [];


    /**
     * @var \Disco\http\Response $response
     */
    private $response;
    ...
    */
    ...
    /**
     * @param string|array $middleware The middle ware to apply to the route.
     *
     * @return $this
     */
    ...
    */
    private function executeRoute(){
        ...
        foreach($this->executedMiddleware as $middleware){
            if(method_exists($middleware, 'terminate')){
                $middleware->terminate(request(), $this->response);
            }
        }
    }
    ...
    */
    private function _executeRoute(){
        ...
        if(!$action instanceof \Closure){
            ...
            if(stripos($action,'@')!==false){
                $ctrl = explode('@',$action);
                if(substr($ctrl[0], 0 , 1) !== '\\'){
                    $ctrl[0] = '\\App\\controller\\' . $ctrl[0];
                }
        ...
        if(is_string($res)){
            ...
            $this->response = new Response(json_encode($res), 200, ['Content-type' => 'application/json']);
        } else if($res === false){
            ...
            throw new \Disco\exceptions\Exception('Router must return a valid string, array, or \Symfony\Component\HttpFoundation\Response');
        }
    ...
    /**
     * Execute all the middleware and middleware groups that were specified on a route. The middleware is processed
     * from first to last as defined by the user and if a middleware doesn't continue the chain then the next middleware
    ...
    private function executeMiddleware(){
        ...
        if($middlewareKey instanceof \Closure){
            return $middlewareKey(request(), function() use($self){
                return $self->executeMiddleware();
            });
        }

        if(array_key_exists($middlewareKey, app()->registeredMiddlewareGroups)) {
            $this->middleware = array_merge(app()->registeredMiddlewareGroups[$middlewareKey], $this->middleware);
            $middlewareKey = array_shift($this->middleware);
        }


        $args = explode(':', $middlewareKey);

        $middlewareName = array_shift($args);

        if(array_key_exists($middlewareName, app()->registeredMiddleware)){
            $middlewareName = app()->registeredMiddleware[$middlewareName];
        }
        ...
        $middleware = app()->with($middlewareName);

        $this->executedMiddleware[] = $middleware;

        return call_user_func_array( [$middleware, 'handle'], $args);
    ...
    */
     ...
     * with the correct answer we need to make sure any un-processed routes are processed first.
     *
     * @param  boolean $m
     *
     * @return boolean
